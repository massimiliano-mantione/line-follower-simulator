...
pub mod devices {
  ...
  impl PollError{
    ...
    pub fn message(&self) -> &'static str {
      match self {
        PollError::InvalidHandle => "The provided handle is not valid
        (this handle has never been created)",
        PollError::ConsumedHandle => "The provided handle has already been consumed
        (the detection of this case is best-effort,
        otherwise the handle is reported as expired)",
        PollError::ExpiredHandle => "The provided handle is too old and its value has been lost
        (more recent values are available and replace the old one)",
      }
    }
  }
...
pub static __WIT_BINDGEN_COMPONENT_TYPE: [u8; 1399] = *b"\
\0asm\x0d\0\x01\0\0\x19\x16wit-component-encoding\x04\0\x07\xed\x09\x01A\x02\x01\
A\x06\x01B\x1b\x01m\x03\x0einvalid-handle\x0fconsumed-handle\x0eexpired-handle\x04\
...
@\x01\x06handle\x05\0\x10\x04\0\x0bdevice-poll\x01\x11\x01@\x01\x05start\x7f\x01\
\0\x04\0\x09poll-loop\x01\x12\x01@\x01\x06handle\x05\x01\0\x04\0\x0dforget-handl\
e\x01\x13\x01@\x02\x04left\x0d\x05right\x0d\x01\0\x04\0\x10set-motors-power\x01\x14\
...
ocessed-by\x02\x0dwit-component\x070.238.0\x10wit-bindgen-rust\x060.45.0";
